...
class Level:
    def __init__(self, levels, bgcolour):
        ...
        self.tumble = []
        self.coins = []
        self.lightcoins = pygame.sprite.Group()
        self.darkcoins = pygame.sprite.Group()
    ...
    def update(self, switch, screen):
        ...
        if self.light:
            for c in self.lightcoins:
                screen.blit(c.image, self.camera.apply(c))
            print "light"
        else:
            for c in self.darkcoins:
                screen.blit(c.image, self.camera.apply(c))
            print "dark"
